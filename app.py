...
def _streamlit_app():
    ...
    if uploaded is not None:
        ...
        # show detected lines (per-line segmentation)
        lines = results.get('lines', [])
        if lines:
            st.subheader('Detected Lines')
            for i, line in enumerate(lines):
                st.write(f'Line {i+1}: rows {line.get("start")}–{line.get("end")}')
                # display original and binary for this line side-by-side
                cols = st.columns(2)
                with cols[0]:
                    st.text('Original')
                    st.image(line['orig'], use_column_width=True)
                    with open(line['orig'], 'rb') as f:
                        st.download_button(f'Download line {i+1} original', f.read(), file_name=f'line_{i+1:02d}_orig.png')
                with cols[1]:
                    st.text('Binary (cleaned)')
                    st.image(line['bin'], use_column_width=True)
                    with open(line['bin'], 'rb') as f:
                        st.download_button(f'Download line {i+1} binary', f.read(), file_name=f'line_{i+1:02d}_bin.png')

        # show combined character/word segments (optional)
        if results.get('combined_segments'):
            st.subheader('Combined Character/Word Segments (grayscale)')
        ...
        if results.get('combined_segments_original'):
            st.subheader('Combined Character/Word Segments (original color)')
